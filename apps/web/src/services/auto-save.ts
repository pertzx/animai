...
const DEFAULT_CONFIG: AutoSaveConfig = {
  // O intervalo é só uma rede de segurança; o gatilho principal é a mudança
  // no JSON global do projeto (markDirty via subscribe no store).
  interval: 30000, // 30 seconds
  maxSlots: 3,
  enabled: true,
  debounceTime: 1000, // 1 second após a última mudança
};
...
class AutoSaveManager {
  ...
  private async saveIfDirty(): Promise<void> {
    // Saves disparados por mudança precisam do estado ATUAL do projeto, não
    // do snapshot do último tick do intervalo.
    if (this.getProject) {
