...
const ANIMAI_PREFS_KEY = "animai.prefs";

export interface AnimaiPrefs {
  /** Instruções personalizadas anexadas ao system prompt do agente. */
  customAiPrompt: string;
  performanceMode: PerformanceMode;
  /** API própria do usuário (BYOK): usa limite de requisições, não saldo. */
  byok: { enabled: boolean; baseUrl: string; model: string; apiKey: string };
}

const DEFAULT_ANIMAI_PREFS: AnimaiPrefs = {
  customAiPrompt: "",
  performanceMode: "auto",
  byok: { enabled: false, baseUrl: "", model: "", apiKey: "" },
};

export function getAnimaiPrefs(): AnimaiPrefs {
  try {
    const raw = localStorage.getItem(ANIMAI_PREFS_KEY);
    return raw
      ? { ...DEFAULT_ANIMAI_PREFS, ...(JSON.parse(raw) as Partial<AnimaiPrefs>) }
      : DEFAULT_ANIMAI_PREFS;
  } catch {
    return DEFAULT_ANIMAI_PREFS;
  }
}

export function setAnimaiPrefs(patch: Partial<AnimaiPrefs>): AnimaiPrefs {
  const next = { ...getAnimaiPrefs(), ...patch };
  localStorage.setItem(ANIMAI_PREFS_KEY, JSON.stringify(next));
  return next;
}
