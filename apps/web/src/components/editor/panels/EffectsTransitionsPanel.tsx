...
import { isLowPowerMode } from "../settings/PerformancePanel";
import type {
  VideoEffectType,
...
// Autoplay (sem hover): ciclo mais lento e ~15fps para não pesar com
// muitos cards visíveis ao mesmo tempo.
const AUTOPLAY_CYCLE_MS = 3600;
const AUTOPLAY_FRAME_GAP_MS = 66;

/**
 * Progresso da animação de preview de um card: roda automaticamente
 * enquanto o card está visível na viewport (IntersectionObserver) e em
 * velocidade/fluidez total no hover.
 */
function usePreviewProgress(pingPong: boolean): {
  ...
  React.useEffect(() => {
    ...
    // Modo desempenho: sem autoplay — preview só no hover.
  ...
  React.useEffect(() => {
    const active = isHover || isVisible;
    if (!active) {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      setProgress(0);
      return;
    }
    const cycle = isHover ? PREVIEW_CYCLE_MS : AUTOPLAY_CYCLE_MS;
    const frameGap = isHover ? 0 : AUTOPLAY_FRAME_GAP_MS;
    ...
    const tick = (now: number) => {
      ...
      setProgress(pingPong ? (t < 0.5 ? t * 2 : (1 - t) * 2) : t);
    };
    rafRef.current = requestAnimationFrame(tick);
    return () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
    };
  }, [isHover, isVisible, pingPong]);
...
const EffectCard: React.FC<{
  ...
  const { ref, progress, hoverProps } = usePreviewProgress(true);
...
const TransitionCard: React.FC<{
  ...
  const { ref, progress, hoverProps } = usePreviewProgress(false);
