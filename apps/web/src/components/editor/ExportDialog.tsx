...
export const ExportDialog: React.FC<ExportDialogProps> = ({
  ...
  // Resoluções rápidas: o rótulo se refere ao lado MENOR do vídeo, então
  // funciona para horizontal, vertical e quadrado (adapta ao aspect ratio).
  ...
  const applyQuickResolution = useCallback(
    (shortSide: number) => {
      const aspect =
      ...
      const width = aspect >= 1 ? even(shortSide * aspect) : shortSide;
      const height = aspect >= 1 ? shortSide : even(shortSide / aspect);
      setCustomSettings((prev) => ({ ...prev, width, height }));
      setActiveTab("custom");
    },
    [projectWidth, projectHeight],
  );
          ...
          <TabsContent value="presets" className="flex-1 overflow-hidden flex mt-0">
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  ...
                  {QUICK_RESOLUTIONS.map((r) => (
                    <button
                      key={r.label}
                      onClick={() => applyQuickResolution(r.shortSide)}
                      title={`Exportar em ${r.label} adaptado ao formato do projeto`}
                      className="rounded-full border border-border px-2.5 py-1 text-xs text-text-secondary transition-colors hover:border-primary hover:text-primary"
                    >
                      {r.label}
                    </button>
                  ))}
          ...
          <div className="flex items-center gap-3">
            {(activeTab === "custom" || selectedPreset) && (
              <span className="text-[11px] text-text-muted">
                ...
                {activeSettings.frameRate}fps •{" "}
                {String(activeSettings.codec).toUpperCase()}
              </span>
            )}
