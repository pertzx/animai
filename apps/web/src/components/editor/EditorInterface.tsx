...
export const EditorInterface: React.FC = () => {
  ...
  return (
    ...
    <div
      ref={rootRef}
      className="min-w-0 flex-1 h-full bg-bg text-fg overflow-hidden font-sans select-none relative z-20 flex flex-col"
    >
      <Toolbar />
    ...
    {/* AnimAI: AI assistant panel (prd.txt §4) */}
    {chatOpen ? (
      <div className="relative z-20 h-full w-[340px] shrink-0 border-l border-border">
        <button
          className="absolute -left-2.5 top-1/2 z-30 flex h-10 w-2.5 -translate-y-1/2 items-center justify-center rounded-l bg-bg-2 text-fg-muted hover:text-fg"
      ...
      <button
        className="relative z-20 h-full w-6 shrink-0 border-l border-border bg-bg-1 text-fg-muted hover:text-accent"
        title="Abrir assistente de IA"
        onClick={() => setChatOpen(true)}
      >
        <span className="mx-auto block rotate-90 whitespace-nowrap text-[10px] tracking-widest">
          ✦ ASSISTENTE
        </span>
      </button>
    )}
