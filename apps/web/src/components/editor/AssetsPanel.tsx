...
import { VECTOR_PRESETS } from "../../services/vector-presets";
import {
  deleteComponent,
  insertComponent,
  listComponents,
  saveComponentFromSelection,
  type ComponentSummary,
} from "../../services/component-library";
...
const CameraSection: React.FC = () => {
  ...
  const apply = async (presetId: string) => {
    setBusy(true);
    try {
      const { applyCameraMove, cameraPresets } = await import(
        "../../services/camera"
      );
      const preset = cameraPresets(Math.min(duration || 5, 10)).find(
        (p) => p.id === presetId,
      );
      if (!preset) return;
      const result = await applyCameraMove(preset.keyframes);
      if (result.ok) {
        toast.success("Câmera aplicada", `${preset.name} (${result.clipsAffected} clipes)`);
      } else {
        toast.warning("Câmera", result.error);
      }
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="mb-6">
      <h4 className="text-xs font-medium text-text-secondary mb-3 flex items-center gap-1.5">
        <Video size={12} />
        Câmera (movimentos de cena)
      </h4>
      <div className="grid grid-cols-2 gap-1.5">
        {[
          ["zoom-in", "Zoom in lento"],
          ["zoom-out", "Zoom out"],
          ["pan-right", "Pan esq → dir"],
          ["punch-in", "Punch-in"],
        ].map(([id, label]) => (
          <button
            key={id}
            ...
            className="rounded-md border border-border bg-background-tertiary px-2 py-2 text-[10.5px] text-text-secondary transition-colors hover:border-primary/60 hover:text-text-primary disabled:opacity-50"
          >
            {label}
          </button>
        ))}
      ...
      <p className="mt-1.5 text-[9.5px] text-text-muted">
        Aplica keyframes de pan/zoom aos clipes visíveis. Peça movimentos
        personalizados ao assistente de IA. Um Ctrl+Z desfaz o movimento.
      </p>
    </div>
  );
};
...
const ComponentsSection: React.FC = () => {
  ...
  const reload = useCallback(() => {
    void listComponents().then(setComponents);
  }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (result.ok) {
        toast.success("Componente salvo", result.component.name);
        setName("");
        reload();
      } else {
        toast.warning("Não foi possível salvar", result.error);
      }
    } finally {
      setBusy(false);
    }
  ...
  const insert = async (id: string) => {
    setBusy(true);
    try {
      const at = useTimelineStore.getState().playheadPosition;
      const result = await insertComponent(id, at);
      if (result.ok) {
        toast.success("Componente inserido", "no playhead atual");
      } else {
        toast.warning("Falha ao inserir", result.error);
      }
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="mb-6">
      <h4 className="text-xs font-medium text-text-secondary mb-3 flex items-center gap-1.5">
        <Package size={12} />
        Componentes (todos os projetos)
      </h4>
      <div className="mb-2 flex gap-1.5">
        <input
          className="min-w-0 flex-1 rounded border border-border bg-background-tertiary px-2 py-1.5 text-xs text-text-primary outline-none focus:border-primary"
          placeholder="Nome do componente"
          value={name}
          onChange={(e) => setName(e.target.value)}
        />
        <button
          className="shrink-0 rounded bg-primary px-2.5 py-1.5 text-xs text-white disabled:opacity-50"
          title="Salva os clipes selecionados na timeline como componente reutilizável"
          disabled={busy}
          onClick={() => void save()}
        >
          Salvar seleção
      ...
      <label className="mb-2 flex items-center gap-1.5 text-[10px] text-text-muted">
        <input
          type="checkbox"
          checked={transparent}
          onChange={(e) => setTransparent(e.target.checked)}
        />
        Fundo transparente (sem clipes de fundo)
      </label>
      {components.length === 0 ? (
        <p className="text-[10px] text-text-muted">
          Selecione clipes na timeline e salve para reutilizar em qualquer
          projeto.
        </p>
      ) : (
        <div className="space-y-1.5">
          {components.map((c) => (
            <div
              key={c.id}
              className="flex items-center gap-2 rounded border border-border bg-background-tertiary px-2 py-1.5 text-xs"
            >
              ...
              <span className="shrink-0 text-[9px] text-text-muted">
                {c.clipCount} clipes · {c.durationSec.toFixed(1)}s
                {c.transparentBackground ? " · transparente" : ""}
              </span>
              <button
                className="shrink-0 text-primary hover:underline disabled:opacity-50"
                disabled={busy}
                onClick={() => void insert(c.id)}
              >
                Inserir
              </button>
              <button
                className="shrink-0 text-text-muted hover:text-red-400"
                onClick={() => void deleteComponent(c.id).then(reload)}
              >
                <Trash2 size={11} />
              </button>
            </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col border-t border-border/70">
            <ScrollArea className="min-h-0 flex-1">
              <div className="px-4 py-4">
                ...
                <div className="mb-6">
                  <button
                    onClick={() => setVectorDrawOpen(true)}
                    ...
                    {VECTOR_PRESETS.map((preset) => (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (newGraphicsTrack) {
                            importSVG(preset.svg, newGraphicsTrack.id, 0);
                          }
                        }}
                        className="aspect-square rounded-md border border-border bg-background-tertiary p-1.5 transition-colors hover:border-primary/60 hover:bg-primary/5"
                      >
                        <img
